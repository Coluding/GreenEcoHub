...
function inferTo(props) {
  const to4 = getForwardProps(props);
  if (to4) {
    const out = {
      to: to4
    };
    eachProp(props, (val, key) => key in to4 || (out[key] = val));
    return out;
  }
...
function isAsyncTo(to4) {
  return is.fun(to4) || is.arr(to4) && is.obj(to4[0]);
}
...
function runAsync(to4, props, state, target) {
  const {
    callId,
    parentId,
    onRest
  ...
  if (!parentId && to4 === prevTo && !props.reset) {
    return prevPromise;
  }
  return state.promise = (async () => {
    ...
    try {
      ...
      if (is.arr(to4)) {
        ...
        animating = Promise.resolve(to4(animate, target.stop.bind(target)));
      }
...
var SpringValue = class extends FrameValue {
  ...
  advance(dt) {
    ...
    anim.values.forEach((node2, i3) => {
      if (node2.done)
        return;
      const to4 = node2.constructor == AnimatedString ? 1 : payload ? payload[i3].lastPosition : toValues[i3];
      ...
      if (!finished) {
        ...
        if (!is.und(config3.duration)) {
          ...
          position = from + config3.easing(p2) * (to4 - from);
          ...
          const precision = config3.precision || (from == to4 ? 5e-3 : Math.min(1, Math.abs(to4 - from) * 1e-3));
          const restVelocity = config3.restVelocity || precision / 10;
          const bounceFactor = config3.clamp ? 0 : config3.bounce;
          const canBounce = !is.und(bounceFactor);
          const isGrowing = from == to4 ? node2.v0 > 0 : from < to4;
          let isMoving;
          ...
          for (let n3 = 0; n3 < numSteps; ++n3) {
            ...
            const springForce = -config3.tension * 1e-6 * (position - to4);
  ...
  start(to4, arg2) {
    let queue;
    if (!is.und(to4)) {
      queue = [is.obj(to4) ? to4 : _extends3({}, arg2, {
        to: to4
      })];
    } else {
      queue = this.queue || [];
  ...
  stop(cancel) {
    ...
    raf.batchedUpdates(() => this._stop(to4, cancel));
    return this;
  }
  reset() {
    this._update({
  ...
  _prepareNode(props) {
    ...
    to4 = is.obj(to4) ? to4[key] : to4;
    if (to4 == null || isAsyncTo(to4)) {
      to4 = void 0;
    }
  ...
  _merge(range, props, resolve) {
    ...
    if (hasFromProp && !hasToProp && (!props.default || is.und(to4))) {
      to4 = from;
    }
    if (props.reverse)
      [to4, from] = [from, to4];
    ...
    from = getFluidValue(from);
    const hasToChanged = !isEqual(to4, prevTo);
    if (hasToChanged) {
      this._focus(to4);
    }
    ...
    const goal = computeGoal(to4);
    const isAnimatable = is.num(goal) || is.arr(goal) || isAnimatedString(goal);
    const immediate = !hasAsyncTo && (!isAnimatable || matchProp(defaultProps.immediate || props.immediate, key));
    if (hasToChanged) {
      const nodeType = getAnimatedType(to4);
    ...
    if (!hasAsyncTo) {
      if (started || hasFluidValue(prevTo)) {
        ...
        anim.toValues = hasFluidValue(to4) ? null : goalType == AnimatedString ? [1] : toArray(goal);
      }
  ...
  _attach() {
    ...
    if (hasFluidValue(to4)) {
      addFluidObserver(to4, this);
      if (isFrameValue(to4)) {
        priority3 = to4.priority + 1;
      }
    }
...
function checkFinished(target, to4) {
  const goal = computeGoal(to4);
  const value = computeGoal(target.get());
  return isEqual(value, goal);
}
function createLoopUpdate(props, loop3 = props.loop, to4 = props.to) {
  ...
  if (loopRet) {
    ...
    return createUpdate(_extends3({}, props, {
      ...
      default: false,
      pause: void 0,
      to: !reverse2 || isAsyncTo(to4) ? to4 : void 0,
...
async function flushUpdate(ctrl, props, isLoop) {
  ...
  if (from === false)
    props.from = null;
  const asyncTo = is.arr(to4) || is.fun(to4) ? to4 : void 0;
  if (asyncTo) {
    props.to = void 0;
    props.onRest = void 0;
    if (defaults3) {
  ...
  if (loop3 && result.finished && !(isLoop && result.noop)) {
    const nextProps = createLoopUpdate(props, loop3, to4);
    if (nextProps) {
...
function hcl(h2, c4, l3, opacity) {
  return arguments.length === 1 ? hclConvert(h2) : new Hcl(h2, c4, l3, opacity == null ? 1 : opacity);
}
function Hcl(h2, c4, l3, opacity) {
  this.h = +h2;
  this.c = +c4;
...
function hue(a3, b3) {
  var d2 = b3 - a3;
  return d2 ? linear2(a3, d2 > 180 || d2 < -180 ? d2 - 360 * Math.round(d2 / 360) : d2) : constant_default(isNaN(a3) ? b3 : a3);
}
function gamma(y4) {
...
function nogamma(a3, b3) {
  var d2 = b3 - a3;
  return d2 ? linear2(a3, d2) : constant_default(isNaN(a3) ? b3 : a3);
}
...
function decompose_default(a3, b3, c4, d2, e4, f2) {
  var scaleX, scaleY, skewX;
  if (scaleX = Math.sqrt(a3 * a3 + b3 * b3))
    a3 /= scaleX, b3 /= scaleX;
  if (skewX = a3 * c4 + b3 * d2)
    c4 -= a3 * skewX, d2 -= b3 * skewX;
...
function hcl2(hue3) {
  return function(start3, end) {
    var h2 = hue3((start3 = hcl(start3)).h, (end = hcl(end)).h), c4 = nogamma(start3.c, end.c), l3 = nogamma(start3.l, end.l), opacity = nogamma(start3.opacity, end.opacity);
    return function(t6) {
      start3.h = h2(t6);
      start3.c = c4(t6);
      start3.l = l3(t6);
...
function makeQueue2() {
  ...
  return {
    add(fn) {
      __raf2.count += current == next && !next.has(fn) ? 1 : 0;
      next.add(fn);
    },
    delete(fn) {
      __raf2.count -= current == next && next.has(fn) ? 1 : 0;
...
function useForceUpdate2() {
  ...
  const mounted = (0, import_react4.useState)(makeMountedRef2)[0];
...
function inferTo2(props) {
  const to4 = getForwardProps2(props);
  if (to4) {
    const out = {
      to: to4
    };
    eachProp2(props, (val, key) => key in to4 || (out[key] = val));
    return out;
  }
...
function isAsyncTo2(to4) {
  return is2.fun(to4) || is2.arr(to4) && is2.obj(to4[0]);
}
...
function runAsync2(to4, props, state, target) {
  const {
    callId,
    parentId,
    onRest
...
var SpringValue2 = class extends FrameValue2 {
  ...
  advance(dt) {
    ...
    anim.values.forEach((node2, i3) => {
      if (node2.done)
        return;
      const to4 = node2.constructor == AnimatedString2 ? 1 : payload ? payload[i3].lastPosition : toValues[i3];
      let finished = anim.immediate;
      let position = to4;
      if (!finished) {
        position = node2.lastPosition;
  ...
  start(to4, arg2) {
    let queue;
    if (!is2.und(to4)) {
      queue = [is2.obj(to4) ? to4 : _extends6({}, arg2, {
        to: to4
      })];
    } else {
      queue = this.queue || [];
  ...
  stop(cancel) {
    ...
    raf2.batchedUpdates(() => this._stop(to4, cancel));
    return this;
  }
  reset() {
    this._update({
  ...
  _prepareNode(props) {
    ...
    to4 = is2.obj(to4) ? to4[key] : to4;
    if (to4 == null || isAsyncTo2(to4)) {
      to4 = void 0;
    }
  ...
  _merge(range, props, resolve) {
    ...
    if (hasFromProp && !hasToProp && (!props.default || is2.und(to4))) {
      to4 = from;
    }
    if (props.reverse)
      [to4, from] = [from, to4];
    ...
    from = getFluidValue2(from);
    const hasToChanged = !isEqual2(to4, prevTo);
    if (hasToChanged) {
      this._focus(to4);
    }
    ...
    if (hasToChanged) {
      const nodeType = getAnimatedType2(to4);
      if (nodeType !== node.constructor) {
        if (immediate) {
          node = this._set(goal);
        } else
    ...
    const goalType = node.constructor;
    let started = hasFluidValue2(to4);
    let finished = false;
    if (!started) {
    ...
    if (!hasAsyncTo) {
      if (started || hasFluidValue2(prevTo)) {
        anim.values = node.getPayload();
        anim.toValues = hasFluidValue2(to4) ? null : goalType == AnimatedString2 ? [1] : toArray2(goal);
      }
      if (anim.immediate != immediate) {
        anim.immediate = immediate;
        if (!immediate && !reset) {
  ...
  _attach() {
    ...
    if (hasFluidValue2(to4)) {
      addFluidObserver2(to4, this);
      if (isFrameValue2(to4)) {
        priority3 = to4.priority + 1;
      }
    }
    this.priority = priority3;
...
function checkFinished2(target, to4) {
  const goal = computeGoal2(to4);
  const value = computeGoal2(target.get());
  return isEqual2(value, goal);
}
function createLoopUpdate2(props, loop3 = props.loop, to4 = props.to) {
  ...
  if (loopRet) {
    ...
    return createUpdate2(_extends6({}, props, {
      ...
      default: false,
      pause: void 0,
      to: !reverse2 || isAsyncTo2(to4) ? to4 : void 0,
      from: reset ? props.from : void 0,
      reset
    }, overrides));
  }
...
async function flushUpdate2(ctrl, props, isLoop) {
  ...
  if (from === false)
    props.from = null;
  const asyncTo = is2.arr(to4) || is2.fun(to4) ? to4 : void 0;
  if (asyncTo) {
    props.to = void 0;
    props.onRest = void 0;
    if (defaults3) {
  ...
  if (loop3 && result.finished && !(isLoop && result.noop)) {
    const nextProps = createLoopUpdate2(props, loop3, to4);
    if (nextProps) {
...
var to3 = (source, ...args) => new Interpolation2(source, args);
globals2.assign({
...
function range_default(start3, stop, step) {
  start3 = +start3, stop = +stop, step = (n3 = arguments.length) < 2 ? (stop = start3, start3 = 0, 1) : n3 < 3 ? 1 : +step;
  var i3 = -1, n3 = Math.max(0, Math.ceil((stop - start3) / step)) | 0, range = new Array(n3);
  while (++i3 < n3) {
...
function band() {
  ...
  function rescale() {
    var n3 = domain().length, reverse2 = r1 < r0, start3 = reverse2 ? r1 : r0, stop = reverse2 ? r0 : r1;
    step = (stop - start3) / Math.max(1, n3 - paddingInner + paddingOuter * 2);
    if (round)
      step = Math.floor(step);
    start3 += (stop - start3 - step * (n3 - paddingInner)) * align;
...
function hcl3(h2, c4, l3, opacity) {
  return arguments.length === 1 ? hclConvert2(h2) : new Hcl2(h2, c4, l3, opacity == null ? 1 : opacity);
}
function Hcl2(h2, c4, l3, opacity) {
  this.h = +h2;
  this.c = +c4;
  this.l = +l3;
  this.opacity = +opacity;
...
function hue2(a3, b3) {
  var d2 = b3 - a3;
  return d2 ? linear4(a3, d2 > 180 || d2 < -180 ? d2 - 360 * Math.round(d2 / 360) : d2) : constant_default3(isNaN(a3) ? b3 : a3);
}
function gamma2(y4) {
...
function nogamma2(a3, b3) {
  var d2 = b3 - a3;
  return d2 ? linear4(a3, d2) : constant_default3(isNaN(a3) ? b3 : a3);
}
...
function numberArray_default2(a3, b3) {
  if (!b3)
    b3 = [];
  var n3 = a3 ? Math.min(b3.length, a3.length) : 0, c4 = b3.slice(), i3;
  return function(t6) {
    for (i3 = 0; i3 < n3; ++i3)
      c4[i3] = a3[i3] * (1 - t6) + b3[i3] * t6;
    return c4;
  };
}
...
function genericArray2(a3, b3) {
  var nb = b3 ? b3.length : 0, na = a3 ? Math.min(nb, a3.length) : 0, x4 = new Array(na), c4 = new Array(nb), i3;
  for (i3 = 0; i3 < na; ++i3)
...
function value_default2(a3, b3) {
  var t6 = typeof b3, c4;
  return b3 == null || t6 === "boolean" ? constant_default3(b3) : (t6 === "number" ? number_default3 : t6 === "string" ? (c4 = color2(b3)) ? (b3 = c4, rgb_default2) : string_default2 : b3 instanceof color2 ? rgb_default2 : b3 instanceof Date ? date_default2 : isNumberArray2(b3) ? numberArray_default2 : Array.isArray(b3) ? genericArray2 : typeof b3.valueOf !== "function" && typeof b3.toString !== "function" || isNaN(b3) ? object_default2 : number_default3)(a3, b3);
}
...
function decompose_default2(a3, b3, c4, d2, e4, f2) {
  var scaleX, scaleY, skewX;
  if (scaleX = Math.sqrt(a3 * a3 + b3 * b3))
    a3 /= scaleX, b3 /= scaleX;
  if (skewX = a3 * c4 + b3 * d2)
...
function hcl4(hue3) {
  return function(start3, end) {
    var h2 = hue3((start3 = hcl3(start3)).h, (end = hcl3(end)).h), c4 = nogamma2(start3.c, end.c), l3 = nogamma2(start3.l, end.l), opacity = nogamma2(start3.opacity, end.opacity);
    return function(t6) {
      start3.h = h2(t6);
      start3.c = c4(t6);
      start3.l = l3(t6);
...
function locale_default(locale3) {
  ...
  function newFormat(specifier) {
    ...
    function format2(value) {
      var valuePrefix = prefix3, valueSuffix = suffix, i3, n3, c4;
      if (type === "c") {
...
function ticker(year2, month2, week, day2, hour2, minute2) {
  ...
  function ticks(start3, stop, count2) {
    ...
    const interval = count2 && typeof count2.range === "function" ? count2 : tickInterval(start3, stop, count2);
...
function formatLocale(locale3) {
  ...
  function newFormat(specifier, formats2) {
    return function(date2) {
      var string = [], i3 = -1, j2 = 0, n3 = specifier.length, c4, pad2, format2;
      if (!(date2 instanceof Date))
        date2 = /* @__PURE__ */ new Date(+date2);
      while (++i3 < n3) {
        if (specifier.charCodeAt(i3) === 37) {
          string.push(specifier.slice(j2, i3));
          if ((pad2 = pads[c4 = specifier.charAt(++i3)]) != null)
  ...
  function parseSpecifier(d2, specifier, string, j2) {
    var i3 = 0, n3 = specifier.length, m3 = string.length, c4, parse;
    while (i3 < n3) {
      if (j2 >= m3)
        return -1;
      ...
      if (c4 === 37) {
        c4 = specifier.charAt(i3++);
        parse = parses[c4 in pads ? specifier.charAt(i3++) : c4];
...
var Zt = function(e4) {
  var r4 = e4.children, t6 = e4.theme, n3 = e4.renderWrapper, o3 = void 0 === n3 || n3, a3 = e4.isInteractive, s3 = void 0 === a3 || a3, l3 = e4.animate, d2 = e4.motionStiffness, u2 = e4.motionDamping, c4 = e4.motionConfig, h2 = (0, import_react8.useRef)(null);
  return (0, import_jsx_runtime.jsx)(Nt, { theme: t6, children: (0, import_jsx_runtime.jsx)($r, { animate: l3, stiffness: d2, damping: u2, config: c4, children: (0, import_jsx_runtime.jsx)(M, { container: h2, children: (0, import_jsx_runtime.jsxs)(Jt, { condition: o3, wrapper: (0, import_jsx_runtime.jsx)("div", { style: Qt, ref: h2 }), children: [r4, s3 && (0, import_jsx_runtime.jsx)(F, {})] }) }) }) });
};
...
var ri = function(e4) {
  var t6 = e4.children, n3 = e4.theme, o3 = e4.isInteractive, a3 = void 0 === o3 || o3, l3 = e4.renderWrapper, d2 = void 0 === l3 || l3, u2 = e4.animate, c4 = e4.motionStiffness, f2 = e4.motionDamping, b3 = e4.motionConfig, y4 = (0, import_react8.useRef)(null), v3 = O(y4), R = v3.actions, q = v3.state, k3 = (0, import_react8.useCallback)(function(e6, r4) {
...
var Ri = (0, import_react8.memo)(function(e4) {
  ...
  var d2, u2 = l3, c4 = l3;
  return 0 === s3 ? d2 = "\n                M 0 0 L " + u2 + " 0\n                M 0 " + c4 + " L " + u2 + " " + c4 + "\n            " : 90 === s3 ? d2 = "\n                M 0 0 L 0 " + c4 + "\n                M " + u2 + " 0 L " + u2 + " " + c4 + "\n            " : (u2 = Math.abs(l3 / Math.sin(ui(s3))), c4 = l3 / Math.sin(ui(90 - s3)), d2 = s3 > 0 ? "\n                    M 0 " + -c4 + " L " + 2 * u2 + " " + c4 + "\n                    M " + -u2 + " " + -c4 + " L " + u2 + " " + c4 + "\n                    M " + -u2 + " 0 L " + u2 + " " + 2 * c4 + "\n                " : "\n                    M " + -u2 + " " + c4 + " L " + u2 + " " + -c4 + "\n                    M " + -u2 + " " + 2 * c4 + " L " + 2 * u2 + " " + -c4 + "\n                    M 0 " + 2 * c4 + " L " + 2 * u2 + " 0\n                "), (0, import_jsx_runtime.jsxs)("pattern", { id: r4, width: u2, height: c4, patternUnits: "userSpaceOnUse", children: [(0, import_jsx_runtime.jsx)("rect", { width: u2, height: c4, fill: n3, stroke: "rgba(255, 0, 0, 0.1)", strokeWidth: 0 }), (0, import_jsx_runtime.jsx)("path", { d: d2, strokeWidth: a3, stroke: o3, strokeLinecap: "square" })] });
});
...
var Ti = function(e4) {
  var r4 = e4.x, t6 = e4.y, i3 = e4.symbol, n3 = void 0 === i3 ? Pi : i3, o3 = e4.size, a3 = e4.datum, s3 = e4.color, l3 = e4.borderWidth, u2 = e4.borderColor, c4 = e4.label, f2 = e4.labelTextAnchor, p2 = void 0 === f2 ? "middle" : f2, h2 = e4.labelYOffset, g2 = void 0 === h2 ? -12 : h2, m3 = Vt(), b3 = tt(), y4 = b3.animate, v3 = b3.config, R = useSpring2({ transform: "translate(" + r4 + ", " + t6 + ")", config: v3, immediate: !y4 });
  return (0, import_jsx_runtime.jsxs)(animated2.g, { transform: R.transform, style: { pointerEvents: "none" }, children: [(0, import_react8.createElement)(n3, { size: o3, color: s3, datum: a3, borderWidth: l3, borderColor: u2 }), c4 && (0, import_jsx_runtime.jsx)("text", { textAnchor: p2, y: g2, style: m3.dots.text, children: c4 })] });
};
...
var ji = function(e4) {
  var r4 = e4.width, t6 = e4.height, i3 = e4.axis, n3 = e4.scale, o3 = e4.value, a3 = e4.lineStyle, s3 = e4.textStyle, l3 = e4.legend, d2 = e4.legendPosition, u2 = e4.legendOffsetX, c4 = e4.legendOffsetY, f2 = e4.legendOrientation, p2 = Vt(), h2 = 0, g2 = 0, m3 = 0, b3 = 0;
  "y" === i3 ? (m3 = n3(o3), g2 = r4) : (h2 = n3(o3), b3 = t6);
  var y4 = null;
  if (l3) {
    var v3 = function(e6) {
      var r5 = e6.axis, t7 = e6.width, i4 = e6.height, n4 = e6.position, o4 = e6.offsetX, a4 = e6.offsetY, s4 = e6.orientation, l4 = 0, d3 = 0, u3 = "vertical" === s4 ? -90 : 0, c5 = "start";
      if ("x" === r5)
        switch (n4) {
          case "top-left":
            l4 = -o4, d3 = a4, c5 = "end";
            break;
          case "top":
            d3 = -a4, c5 = "horizontal" === s4 ? "middle" : "start";
            break;
          case "top-right":
            l4 = o4, d3 = a4, c5 = "horizontal" === s4 ? "start" : "end";
            break;
          case "right":
            l4 = o4, d3 = i4 / 2, c5 = "horizontal" === s4 ? "start" : "middle";
            break;
          case "bottom-right":
            l4 = o4, d3 = i4 - a4, c5 = "start";
            break;
          case "bottom":
            d3 = i4 + a4, c5 = "horizontal" === s4 ? "middle" : "end";
            break;
          case "bottom-left":
            d3 = i4 - a4, l4 = -o4, c5 = "horizontal" === s4 ? "end" : "start";
            break;
          case "left":
            l4 = -o4, d3 = i4 / 2, c5 = "horizontal" === s4 ? "end" : "middle";
        }
      else
        switch (n4) {
          case "top-left":
            l4 = o4, d3 = -a4, c5 = "start";
            break;
          case "top":
            l4 = t7 / 2, d3 = -a4, c5 = "horizontal" === s4 ? "middle" : "start";
            break;
          case "top-right":
            l4 = t7 - o4, d3 = -a4, c5 = "horizontal" === s4 ? "end" : "start";
            break;
          case "right":
            l4 = t7 + o4, c5 = "horizontal" === s4 ? "start" : "middle";
            break;
          case "bottom-right":
            l4 = t7 - o4, d3 = a4, c5 = "end";
            break;
          case "bottom":
            l4 = t7 / 2, d3 = a4, c5 = "horizontal" === s4 ? "middle" : "end";
            break;
          case "bottom-left":
            l4 = o4, d3 = a4, c5 = "horizontal" === s4 ? "start" : "end";
            break;
          case "left":
            l4 = -o4, c5 = "horizontal" === s4 ? "end" : "middle";
        }
      return { x: l4, y: d3, rotation: u3, textAnchor: c5 };
    }({ axis: i3, width: r4, height: t6, position: d2, offsetX: u2, offsetY: c4, orientation: f2 });
...
var rn = function(e4, r4, t6, i3) {
  var n3 = void 0 === i3 ? {} : i3, o3 = n3.dataKey, a3 = n3.colorKey, s3 = void 0 === a3 ? "color" : a3, l3 = n3.targetKey, d2 = void 0 === l3 ? "fill" : l3, u2 = [], c4 = {};
        ...
        if (f2) {
          if ($i.includes(f2.type))
            if ("inherit" === f2.background || "inherit" === f2.color) {
              ...
              "inherit" === f2.background && (m3 = m3 + ".bg." + p2, h2 = p2), "inherit" === f2.color && (m3 = m3 + ".fg." + p2, g2 = p2), (0, import_set2.default)(r5, d2, "url(#" + m3 + ")"), c4[m3] || (u2.push(Gr({}, f2, { id: m3, background: h2, color: g2 })), c4[m3] = 1);
            } else
            ...
            if (f2.colors.map(function(e6) {
              ...
              q.id = R, (0, import_set2.default)(r5, d2, "url(#" + R + ")"), c4[R] || (u2.push(q), c4[R] = 1);
            } else
              (0, import_set2.default)(r5, d2, "url(#" + a4 + ")");
          }
        }
...
var m2 = function(t6, o3) {
  return "translate(" + t6 + "px, " + o3 + "px)";
};
var g = (0, import_react9.memo)(function(t6) {
  var i3, n3 = t6.position, r4 = t6.anchor, e4 = t6.children, l3 = Vt(), d2 = tt(), p2 = d2.animate, f2 = d2.config, g2 = Ft(), b3 = g2[0], w2 = g2[1], T2 = (0, import_react9.useRef)(false), C5 = void 0, E4 = false, P3 = w2.width > 0 && w2.height > 0, N2 = Math.round(n3[0]), V2 = Math.round(n3[1]);
  ...
  var j2 = useSpring({ to: C5, config: f2, immediate: !p2 || E4 }), O4 = v2({}, x3, l3.tooltip, { transform: null != (i3 = j2.transform) ? i3 : m2(N2, V2) });
...
var b2 = (0, import_react9.memo)(function(t6) {
  var o3 = t6.size, i3 = void 0 === o3 ? 12 : o3, n3 = t6.color, r4 = t6.style;
  return (0, import_jsx_runtime2.jsx)("span", { style: v2({ display: "block", width: i3, height: i3, background: n3 }, void 0 === r4 ? {} : r4) });
});
var w = (0, import_react9.memo)(function(t6) {
  var o3, i3 = t6.id, n3 = t6.value, r4 = t6.format, e4 = t6.enableChip, l3 = void 0 !== e4 && e4, a3 = t6.color, c4 = t6.renderContent, h2 = Vt(), u2 = Xt(r4);
  if ("function" == typeof c4)
    o3 = c4();
  else {
    var f2 = n3;
    void 0 !== u2 && void 0 !== f2 && (f2 = u2(f2)), o3 = (0, import_jsx_runtime2.jsxs)("div", { style: h2.tooltip.basic, children: [l3 && (0, import_jsx_runtime2.jsx)(b2, { color: a3, style: h2.tooltip.chip }), void 0 !== f2 ? (0, import_jsx_runtime2.jsxs)("span", { children: [i3, ": ", (0, import_jsx_runtime2.jsx)("strong", { children: "" + f2 })] }) : i3] });
  }
  return (0, import_jsx_runtime2.jsx)("div", { style: h2.tooltip.container, children: o3 });
});
var T = { width: "100%", borderCollapse: "collapse" };
var C4 = (0, import_react9.memo)(function(t6) {
  var o3, i3 = t6.title, n3 = t6.rows, r4 = void 0 === n3 ? [] : n3, e4 = t6.renderContent, l3 = Vt();
  return r4.length ? (o3 = "function" == typeof e4 ? e4() : (0, import_jsx_runtime2.jsxs)("div", { children: [i3 && i3, (0, import_jsx_runtime2.jsx)("table", { style: v2({}, T, l3.tooltip.table), children: (0, import_jsx_runtime2.jsx)("tbody", { children: r4.map(function(t7, o4) {
    return (0, import_jsx_runtime2.jsx)("tr", { children: t7.map(function(t8, o5) {
      return (0, import_jsx_runtime2.jsx)("td", { style: l3.tooltip.tableCell, children: t8 }, o5);
    }) }, o4);
  }) }) })] }), (0, import_jsx_runtime2.jsx)("div", { style: l3.tooltip.container, children: o3 })) : null;
});
C4.displayName = "TableTooltip";
var E3 = (0, import_react9.memo)(function(t6) {
  var o3 = t6.x0, n3 = t6.x1, r4 = t6.y0, e4 = t6.y1, l3 = Vt(), u2 = tt(), d2 = u2.animate, p2 = u2.config, f2 = (0, import_react9.useMemo)(function() {
    return v2({}, l3.crosshair.line, { pointerEvents: "none" });
  }, [l3.crosshair.line]), x4 = useSpring({ x1: o3, x2: n3, y1: r4, y2: e4, config: p2, immediate: !d2 });
...
var P2 = (0, import_react9.memo)(function(t6) {
  var o3, i3, n3 = t6.width, r4 = t6.height, e4 = t6.type, l3 = t6.x, a3 = t6.y;
  return "cross" === e4 ? (o3 = { x0: l3, x1: l3, y0: 0, y1: r4 }, i3 = { x0: 0, x1: n3, y0: a3, y1: a3 }) : "top-left" === e4 ? (o3 = { x0: l3, x1: l3, y0: 0, y1: a3 }, i3 = { x0: 0, x1: l3, y0: a3, y1: a3 }) : "top" === e4 ? o3 = { x0: l3, x1: l3, y0: 0, y1: a3 } : "top-right" === e4 ? (o3 = { x0: l3, x1: l3, y0: 0, y1: a3 }, i3 = { x0: l3, x1: n3, y0: a3, y1: a3 }) : "right" === e4 ? i3 = { x0: l3, x1: n3, y0: a3, y1: a3 } : "bottom-right" === e4 ? (o3 = { x0: l3, x1: l3, y0: a3, y1: r4 }, i3 = { x0: l3, x1: n3, y0: a3, y1: a3 }) : "bottom" === e4 ? o3 = { x0: l3, x1: l3, y0: a3, y1: r4 } : "bottom-left" === e4 ? (o3 = { x0: l3, x1: l3, y0: a3, y1: r4 }, i3 = { x0: 0, x1: l3, y0: a3, y1: a3 }) : "left" === e4 ? i3 = { x0: 0, x1: l3, y0: a3, y1: a3 } : "x" === e4 ? o3 = { x0: l3, x1: l3, y0: 0, y1: r4 } : "y" === e4 && (i3 = { x0: 0, x1: n3, y0: a3, y1: a3 }), (0, import_jsx_runtime2.jsxs)(import_jsx_runtime2.Fragment, { children: [o3 && (0, import_jsx_runtime2.jsx)(E3, { x0: o3.x0, x1: o3.x1, y0: o3.y0, y1: o3.y1 }), i3 && (0, import_jsx_runtime2.jsx)(E3, { x0: i3.x0, x1: i3.x1, y0: i3.y0, y1: i3.y1 })] });
});
...
var O = function(t6) {
  var o3 = (0, import_react9.useState)(V), n3 = o3[0], l3 = o3[1], a3 = (0, import_react9.useCallback)(function(t7, o4, i3) {
    var n4 = o4[0], r4 = o4[1];
    void 0 === i3 && (i3 = "top"), l3({ isVisible: true, position: [n4, r4], anchor: i3, content: t7 });
  }, [l3]), c4 = (0, import_react9.useCallback)(function(o4, i3, n4) {
    void 0 === n4 && (n4 = "top");
    var r4 = t6.current.getBoundingClientRect(), e4 = i3.clientX - r4.left, a4 = i3.clientY - r4.top;
    "left" !== n4 && "right" !== n4 || (n4 = e4 < r4.width / 2 ? "right" : "left"), l3({ isVisible: true, position: [e4, a4], anchor: n4, content: o4 });
  }, [t6, l3]), s3 = (0, import_react9.useCallback)(function() {
    l3(V);
  }, [l3]);
  return { actions: (0, import_react9.useMemo)(function() {
    return { showTooltipAt: a3, showTooltipFromEvent: c4, hideTooltip: s3 };
  }, [a3, c4, s3]), state: n3 };
};
var k2 = function() {
...
var M = function(t6) {
  var o3 = t6.container, i3 = t6.children, n3 = O(o3), r4 = n3.actions, e4 = n3.state;
  return (0, import_jsx_runtime2.jsx)(N.Provider, { value: r4, children: (0, import_jsx_runtime2.jsx)(j.Provider, { value: e4, children: i3 }) });
};
...
//# sourceMappingURL=chunk-SSDMV6MA.js.map
